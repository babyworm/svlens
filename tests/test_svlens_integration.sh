...
if ! grep -q 'svlens v0.3.3' <<<"$SVLENS_HELP"; then
    echo "FAIL: svlens --help missing banner"
    exit 1
fi
SVLENS_VERSION="$("$SVLENS_BINARY" --version)"
if ! grep -q '^svlens 0\.3\.3$' <<<"$SVLENS_VERSION"; then
...
if ! grep -q 'svlens conn v0.3.3' "$OUTDIR/svlens_conn_help.txt"; then
    echo "FAIL: svlens conn --help missing banner"
    exit 1
fi
"$SVLENS_BINARY" conn --version >"$OUTDIR/svlens_conn_version.txt"
if ! grep -q '^svlens conn 0\.3\.3$' "$OUTDIR/svlens_conn_version.txt"; then
...
if ! grep -q 'svlens cdc v0.3.3' "$OUTDIR/svlens_cdc_help.txt"; then
    echo "FAIL: svlens cdc --help missing banner"
    exit 1
fi
"$SVLENS_BINARY" cdc --version >"$OUTDIR/svlens_cdc_version.txt"
if ! grep -q '^svlens cdc 0\.3\.3$' "$OUTDIR/svlens_cdc_version.txt"; then
