...
if ! grep -q 'svlens v0.2.2' <<<"$SVLENS_HELP"; then
    echo "FAIL: svlens --help missing banner"
    exit 1
fi
SVLENS_VERSION="$("$SVLENS_BINARY" --version)"
if ! grep -q '^svlens 0\.2\.2$' <<<"$SVLENS_VERSION"; then
...
if ! grep -q 'svlens conn v0.2.2' /tmp/svlens_conn_help.txt; then
    echo "FAIL: svlens conn --help missing banner"
    exit 1
fi
"$SVLENS_BINARY" conn --version >/tmp/svlens_conn_version.txt
if ! grep -q '^svlens conn 0\.2\.2$' /tmp/svlens_conn_version.txt; then
...
if ! grep -q 'svlens cdc v0.2.2' /tmp/svlens_cdc_help.txt; then
    echo "FAIL: svlens cdc --help missing banner"
    exit 1
fi
"$SVLENS_BINARY" cdc --version >/tmp/svlens_cdc_version.txt
if ! grep -q '^svlens cdc 0\.2\.2$' /tmp/svlens_cdc_version.txt; then
